...
At this time, flags and features are one-to-one and share a common key. Using the [check flag endpoint](/api-reference/features/check-flag), you may evaluate features and flags using a company and/or user key.
...
You can view [API Logs](https://app.schematichq.com/settings/api-logs) within Settings to see both successful and unsuccessful requests. Within each request, you can view detail describing the response code, request ID, start/end times, the API key used, the method, the URL path, and the request & response payloads.
