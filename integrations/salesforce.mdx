...
* Create and update company and user traits
...
Under External Credential Principal Access, select Edit. Find the principal you created under Available External Credential Principals and add so that it is under the Enabled External Credential Principals.
...
We will need to generate an authorization header to include your API key from Schematic, so ensure that is checked.
...
* X-Schematic-Api-Key ... ```your-api-key```
...
On the next screen in the wizard, label the action as PostCompany with a method of POST (if we were retrieving information from Schematic, we would set the method to GET). Add a short description if you desire.

<img
  src="/images/salesforce_integration_9.png"
/>

Set the URL path to [/companies](/api-reference/companies/create-company), which corresponds to the endpoint to create and update companies. Use the following sample request in the Sample Request section (for other endpoints, refer to the [API Reference](/api-reference)).
...
Click on Sample Response and copy the response [here](/api-reference/companies/create-company) for 201s (copied below).
...
Save the HTTP Callout. In the New Action Modal, you will be prompted to add a label and API name, then set the request body.

In the dropdown click "New Resource" and name the resource based on the auto-selected Apex Class. The Data Type and Apex Class should be grayed out like below.
...
Add an Assignment element prior to the HTTP Callout in your flow to assign values to the variables from your sample request. Map each variable from the sample request.
...
In this example, we'll use static values, but you can also reference objects within Salesforce to assign values dynamically if you retreive them prior to assignment in the Flow Builder. 

### Run the flow

Add a Screen element after the HTTP Callout action in your Flow to observe output.
