...
* X-Schematic-Api-Key ... ```our-api-key```
...
If the API call is working as expected, we can now customize the flow in a variety of ways to retrieve dynamic values from Salesforce, to trigger based on activity (e.g. a closed won deal), or to generate requests against Schematic endpoints.