...
1. **Batched upserts to companies/users within Schematic.** Often, Tthis occurs if a daily cron job is set up to update traits within Schematic. We suggest either spacing out requests to our API or transitioning upserts to using identify calls via our event capture service.
...
We also recommend implementign local caching with a TTL (either via our SDK or implemented independently).